...
def emissions(run_start_year, run_end_year, dt, rcp, add_start = 0, 
    ...
    for t in range(0,int(steps)):
        df.loc[df.index[t], 'year'] = subset['year'][int(df['date'][t])]
        df.loc[df.index[t], 'co2_pg'] = subset['c_emissions_pg'][int(df['date'][t])] * dt * C_TO_CO2
        df.loc[df.index[t], 'ch4_tg'] = subset['ch4_emissions_tg'][int(df['date'][t])] * dt
        df.loc[df.index[t], 'n2o_tg'] = subset['n2o_emissions_tg'][int(df['date'][t])] * dt
        df.loc[df.index[t], 'hist_forcing_wm2'] = subset['hist_forcing_wm2'][int(df['date'][t])]
        df.loc[df.index[t], 'co2_forcing_rcp'] = subset['co2_forcing_wm2'][int(df['date'][t])]
        df.loc[df.index[t], 'ch4_forcing_rcp'] = subset['ch4_forcing_wm2'][int(df['date'][t])]
        df.loc[df.index[t], 'n2o_forcing_rcp'] = subset['n2o_forcing_wm2'][int(df['date'][t])]
        df.loc[df.index[t], 'total_forcing_rcp'] = subset['total_forcing_wm2'][int(df['date'][t])]
        df.loc[df.index[t], 'rcp_co2_ppm'] = subset['co2_concentration_ppm'][int(df['date'][t])]
        df.loc[df.index[t], 'rcp_ch4_ppb'] = subset['ch4_concentration_ppb'][int(df['date'][t])]
        df.loc[df.index[t], 'rcp_n2o_ppb'] = subset['n2o_concentration_ppb'][int(df['date'][t])]
